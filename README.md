...
- **Software dependencies** — see [md_files/SETUP.md](md_files/SETUP.md) for the full list. Mitra lab members on HTCF: many packages are already installed at `/ref/rmlab/software/`, but PLEASE still double check them. Otherwise, you can just proceed to step 2 below for the installation.
...
```
Customize all paths and emails across every file in this project — r_install/ shell scripts, nextflow/ shell scripts (submit.sh, run.sh), all R scripts, all Nextflow .nf files and nextflow.config, and all md files — for my HTCF account.
My project root is:   <your path>
