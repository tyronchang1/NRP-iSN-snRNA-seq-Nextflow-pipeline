...
#SBATCH --job-name=r_install_cran
#SBATCH --output=r_install/logs/01_cran_%j.out
#SBATCH --error=r_install/logs/01_cran_%j.err
#SBATCH --time=4:00:00
#SBATCH --mem=64G
...
# devtools and tidyverse excluded: both pull in ragg -> textshaping -> systemfonts ->
# fontconfig which is not on this cluster. Individual tidyverse components installed instead.
pkgs <- c(
  "BiocManager", "remotes", "pak",
  "ggplot2", "dplyr", "tidyr", "readr", "purrr", "tibble", "stringr", "forcats",
  "lubridate", "scales", "broom",
  "knitr", "lintr", "styler", "here", "markdown", "rmarkdown",
  "Seurat", "SoupX", "cowplot", "scCustomize", "viridis", "ggpubr", "clustree", "magick",
  "randomcoloR", "gt", "pheatmap",
  "msigdbr", "RobustRankAggreg", "shiny", "reticulate", "data.table", "Matrix",
  "DT", "magrittr", "ggrepel", "ggdendro", "gridExtra", "glue", "readr",
  "future", "RColorBrewer", "R.utils", "patchwork", "tictoc", "callback",
  "RSpectra"
)

failed <- character(0)

install_one <- function(pkg, lib, repos) {
  for (attempt in 1:3) {
    ok <- tryCatch({
...
if (length(failed) > 0) {
  ...
  writeLines(failed, file.path(lib, "failed_01_cran.txt"))
  # Only abort the chain if critical tools are missing
  critical <- c("BiocManager", "remotes")
  if (length(intersect(failed, critical)) > 0) {
    cat("CRITICAL packages failed — stopping chain\n")
    quit(status = 1)
  }
} else {
  cat("\n=== All CRAN packages installed successfully ===\n")
  f <- file.path(lib, "failed_01_cran.txt")
