...
sc <- load10X('./samples/NR00_Day13_1')
...
p1 <- ggplot(dd, aes(tSNE1, tSNE2)) +
  ...
  ggtitle("NR00_Day13_1 study pre-Soup") + center           # title identifies sample and that this is before correction
...
out_NR00_Day13_1 <- adjustCounts(sc, roundToInt = TRUE)
out_NR00_Day13_1 # print summary of corrected matrix
...
corrected_tubb3  <- plotChangeMap(sc, out_NR00_Day13_1, "TUBB3") + ggtitle("TUBB3, NR00_Day13_1 post-Soup") + center
corrected_prph   <- plotChangeMap(sc, out_NR00_Day13_1, "PRPH")  + ggtitle("PRPH, NR00_Day13_1 post-Soup")  + center
corrected_ntrk2  <- plotChangeMap(sc, out_NR00_Day13_1, "NTRK2") + ggtitle("NTRK2, NR00_Day13_1 post-Soup") + center
corrected_calca  <- plotChangeMap(sc, out_NR00_Day13_1, "CALCA") + ggtitle("CALCA, NR00_Day13_1 post-Soup") + center
...
cntStrained <- rowSums(out_NR00_Day13_1 > 0)    # post-Soup: same count after correction
...
mostZeroed  <- tail(sort((cntSoggy - cntStrained) / cntSoggy), n = 100) # top 10 most-affected genes
mostZeroed
...
tail(sort(rowSums(sc$toc > out_NR00_Day13_1) / rowSums(sc$toc > 0)), n = 100) # top 20 highest soup-fraction genes
...
dir.create("./scripts/01_SoupX/SoupX_dir_out", recursive = TRUE)
unlink("./scripts/01_SoupX/SoupX_dir_out/NR00_Day13_1Counts", recursive = TRUE)
DropletUtils:::write10xCounts("./scripts/01_SoupX/SoupX_dir_out/NR00_Day13_1Counts", out_NR00_Day13_1)
