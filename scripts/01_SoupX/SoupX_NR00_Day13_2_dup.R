...
sc <- load10X('./samples/NR00_Day13_2_dup')
...
p1 <- ggplot(dd, aes(tSNE1, tSNE2)) +
  ...
  ggtitle("NR00_Day13_2_dup study pre-Soup") + center       # title identifies sample and that this is before correction
...
genes_check <- c("TUBB3", "PRPH", "NTRK2", "CALCA", "POU5F1", "SOX2", "STMN2","MAP2","SNAP25",
                 "RBFOX3","NEFH","ACTB")
...
out_NR00_Day13_2_dup <- adjustCounts(sc, roundToInt = TRUE)
out_NR00_Day13_2_dup # print summary of corrected matrix
...
corrected_tubb3  <- plotChangeMap(sc, out_NR00_Day13_2_dup, "TUBB3") + ggtitle("TUBB3, NR00_Day13_2_dup post-Soup") + center
corrected_prph   <- plotChangeMap(sc, out_NR00_Day13_2_dup, "PRPH")  + ggtitle("PRPH, NR00_Day13_2_dup post-Soup")  + center
corrected_ntrk2  <- plotChangeMap(sc, out_NR00_Day13_2_dup, "NTRK2") + ggtitle("NTRK2, NR00_Day13_2_dup post-Soup") + center
corrected_snap25 <- plotChangeMap(sc, out_NR00_Day13_2_dup, "SNAP25") + ggtitle("SNAP25, NR00_Day13_2_dup post-Soup") + center
strained_p <- plot_grid(corrected_tubb3, corrected_prph, corrected_ntrk2, corrected_snap25) # arrange all 4 panels in a grid
strained_p
...
cntStrained <- rowSums(out_NR00_Day13_2_dup > 0)    # post-Soup: same count after correction
...
tail(sort(rowSums(sc$toc > out_NR00_Day13_2_dup) / rowSums(sc$toc > 0)), n = 100) # top 100 highest soup-fraction genes
...
unlink("./scripts/01_SoupX/SoupX_dir_out/NR00_Day13_2_dupCounts", recursive = TRUE)
DropletUtils:::write10xCounts("./scripts/01_SoupX/SoupX_dir_out/NR00_Day13_2_dupCounts", out_NR00_Day13_2_dup)
