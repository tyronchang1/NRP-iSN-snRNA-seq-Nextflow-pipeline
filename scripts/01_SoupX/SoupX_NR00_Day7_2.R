...
sc <- load10X('./samples/NR00_Day7_2')
...
p1 <- ggplot(dd, aes(tSNE1, tSNE2)) +
  ...
  ggtitle("NR00_Day7_2 study pre-Soup") + center            # title identifies sample and that this is before correction
...
out_NR00_Day7_2 <- adjustCounts(sc, roundToInt = TRUE)
out_NR00_Day7_2 # print summary of corrected matrix
...
corrected_tubb3  <- plotChangeMap(sc, out_NR00_Day7_2, "TUBB3") + ggtitle("TUBB3, NR00_Day7_2 post-Soup") + center
corrected_prph   <- plotChangeMap(sc, out_NR00_Day7_2, "PRPH")  + ggtitle("PRPH, NR00_Day7_2 post-Soup")  + center
corrected_ntrk2  <- plotChangeMap(sc, out_NR00_Day7_2, "NTRK2") + ggtitle("NTRK2, NR00_Day7_2 post-Soup") + center
corrected_calca  <- plotChangeMap(sc, out_NR00_Day7_2, "CALCA") + ggtitle("CALCA, NR00_Day7_2 post-Soup") + center
...
cntStrained <- rowSums(out_NR00_Day7_2 > 0)    # post-Soup: same count after correction
...
tail(sort(rowSums(sc$toc > out_NR00_Day7_2) / rowSums(sc$toc > 0)), n = 200) # top 200 highest soup-fraction genes
...
unlink("./scripts/01_SoupX/SoupX_dir_out/NR00_Day7_2Counts", recursive = TRUE)
DropletUtils:::write10xCounts("./scripts/01_SoupX/SoupX_dir_out/NR00_Day7_2Counts", out_NR00_Day7_2)
