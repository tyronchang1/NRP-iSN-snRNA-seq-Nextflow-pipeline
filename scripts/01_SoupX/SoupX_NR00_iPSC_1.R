...
sc <- load10X('./samples/NR00_iPSC_1')
...
print(sc)   # summary:Channel with 36601 genes and 13961 cells
str(sc)     # full internal structure 
...
dd <- sc_metadata # working copy used for plotting; keeps sc$metaData unchanged #(use metadata to cluster)
...
p1 <- ggplot(dd, aes(tSNE1, tSNE2)) +
  ...
  ggtitle("NR00_iPSC_1 study pre-Soup") + center            # title identifies sample and that this is before correction
...
# iPSC pluripotency markers: POU5F1 (OCT4), SOX2, NANOG
# these confirm undifferentiated iPSC identity and help distinguish residual undifferentiated cells from iSNs

# POU5F1 (OCT4): master pluripotency transcription factor; high expression = undifferentiated iPSC
dd$POU5F1 = sc$toc["POU5F1", ]
pou5f1_p <- ggplot(dd, aes(tSNE1, tSNE2)) + geom_point(aes(colour = POU5F1 > 0))
plot(pou5f1_p)
plotMarkerMap(sc, "POU5F1")

# SOX2: pluripotency and neural progenitor marker; co-expressed with OCT4 in iPSCs
dd$SOX2 = sc$toc["SOX2", ]
sox2_p <- ggplot(dd, aes(tSNE1, tSNE2)) + geom_point(aes(colour = SOX2 > 0))
plot(sox2_p)
plotMarkerMap(sc, "SOX2")

# NANOG: core pluripotency network member; maintains self-renewal; lost upon differentiation
dd$NANOG = sc$toc["NANOG", ]
nanog_p <- ggplot(dd, aes(tSNE1, tSNE2)) + geom_point(aes(colour = NANOG > 0))
plot(nanog_p)
plotMarkerMap(sc, "NANOG")
...
data.frame(
  ...
  pct_lost = round((cntSoggy[genes_check] - cntStrained[genes_check])/cntSoggy[genes_check] * 100, 1)
)
...
out_NR00_iPSC_1 <- adjustCounts(sc, roundToInt = TRUE)
out_NR00_iPSC_1 # print summary of corrected matrix
...
corrected_tubb3  <- plotChangeMap(sc, out_NR00_iPSC_1, "TUBB3")  + ggtitle("TUBB3, NR00_iPSC_1 post-Soup")  + center
corrected_prph   <- plotChangeMap(sc, out_NR00_iPSC_1, "PRPH")   + ggtitle("PRPH, NR00_iPSC_1 post-Soup")   + center
corrected_ntrk2  <- plotChangeMap(sc, out_NR00_iPSC_1, "NTRK2")  + ggtitle("NTRK2, NR00_iPSC_1 post-Soup")  + center
corrected_calca  <- plotChangeMap(sc, out_NR00_iPSC_1, "CALCA")  + ggtitle("CALCA, NR00_iPSC_1 post-Soup")  + center
corrected_pou5f1 <- plotChangeMap(sc, out_NR00_iPSC_1, "POU5F1") + ggtitle("POU5F1, NR00_iPSC_1 post-Soup") + center
corrected_sox2   <- plotChangeMap(sc, out_NR00_iPSC_1, "SOX2")   + ggtitle("SOX2, NR00_iPSC_1 post-Soup")   + center
corrected_nanog  <- plotChangeMap(sc, out_NR00_iPSC_1, "NANOG")  + ggtitle("NANOG, NR00_iPSC_1 post-Soup")  + center
strained_p <- plot_grid(corrected_tubb3, corrected_prph, corrected_ntrk2, corrected_calca,
                        corrected_pou5f1, corrected_sox2, corrected_nanog) # arrange all 7 panels in a grid
strained_p
...
cntStrained <- rowSums(out_NR00_iPSC_1 > 0)     # post-Soup: same count after correction
...
tail(sort(rowSums(sc$toc > out_NR00_iPSC_1) / rowSums(sc$toc > 0)), n = 200) # top 20 highest soup-fraction genes
head(sort(rowSums(sc$toc > out_NR00_iPSC_1) / rowSums(sc$toc > 0)), n = 200) # top 20 highest soup-fraction genes
...
unlink("./scripts/01_SoupX/SoupX_dir_out/NR00_iPSC_1Counts", recursive = TRUE)
